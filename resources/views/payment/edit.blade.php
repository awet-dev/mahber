        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('users.payments.update', ['user' => $user->id, 'payment' => $payment->id]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="amount_paid" class="block mt-1 w-full" name="amount_paid"
                                          :value="old('amount_paid', $payment->amount_paid)" required autofocus/>
                        ...
                        <div>
                            ...
                            <x-text-input id="amount_left" class="block mt-1 w-full" name="amount_left"
                                          :value="old('amount_left', $payment->amount_left)"/>
                            <x-input-error :messages="$errors->get('amount_left')" class="mt-2"/>
                        </div>

                        <!-- Amount Return -->
                        <div>
                            <x-input-label for="amount_back" :value="__('Amount Return')"/>
                            <x-text-input id="amount_back" class="block mt-1 w-full" name="amount_back"
                                          :value="old('amount_back', $payment->amount_back)"/>
